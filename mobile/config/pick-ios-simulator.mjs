...
for (const [runtime, devices] of Object.entries(data.devices)) {
    ...
    if (iphone) {
        found = { name: iphone.name, udid: iphone.udid, runtime };
        break;
    }
}
...
console.log(`Using simulator: ${found.name} (${found.udid}) / iOS ${platformVersion}`);
...
fs.appendFileSync(envFile, `MOBILE_IOS_DEVICE_UDID=${found.udid}\n`);
